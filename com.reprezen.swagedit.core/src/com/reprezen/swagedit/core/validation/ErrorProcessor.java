...
public class ErrorProcessor {
    ...
    public ErrorProcessor(JsonModel model, JsonNode jsonSchema) {
        this.model = model;
    ...
    private SwaggerError createUnique(JsonNode error, int indent) {
        final SwaggerError schemaError = new SwaggerError(getLine(error, model), getLevel(error), indent,
                rewriteError(error));
    ...
    private SwaggerError createMultiple(JsonNode error, int indent) {
        final MultipleSwaggerErrorBuilder schemaErrorBuilder = new MultipleSwaggerErrorBuilder()
                .locatedOn(getLine(error, model)).withSeverity(getLevel(error)).indented(indent).basedOnSchema(schema);
