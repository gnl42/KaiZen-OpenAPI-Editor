...
public class JsonProposalProvider {
     ...
     * @return proposals
     */
    public Collection<ProposalBuilder> getProposals(JsonPointer pointer, Model model, String prefix) {
     ...
     * @return proposals
     */
    public Collection<ProposalBuilder> getProposals(JsonPointer pointer, Model model) {
     ...
     * @return proposals
     */
    public Collection<ProposalBuilder> getProposals(AbstractNode node) {
        return getProposals(node.getType(), node, null);
    }

    protected Collection<ProposalBuilder> getProposals(TypeDefinition type, AbstractNode node, String prefix) {
        ...
        case UNDEFINED:
            Collection<ProposalBuilder> proposals = new LinkedHashSet<>();
    ...
    protected Collection<ProposalBuilder> createPrimitiveProposals(TypeDefinition type) {
        String label;
        ...
        return Lists.newArrayList(new ProposalBuilder("").replacementString("").description(type.getDescription()).type(label));
    }

    protected Collection<ProposalBuilder> createBooleanProposals(TypeDefinition type) {
        Collection<ProposalBuilder> proposals = new LinkedHashSet<>();

        String labelType = type.getType().getValue();

        proposals.add(new ProposalBuilder("true").replacementString("true").description(type.getDescription()).type(labelType));
        proposals.add(new ProposalBuilder("false").replacementString("false").description(type.getDescription()).type(labelType));

        return proposals;
    }

    protected ProposalBuilder createPropertyProposal(String key, TypeDefinition type) {
        ...
        return new ProposalBuilder(key).replacementString(key + ":").description(type.getDescription()).type(labelType);
    }

    protected Collection<ProposalBuilder> createObjectProposals(ObjectTypeDefinition type, AbstractNode element,
            String prefix) {
        final Collection<ProposalBuilder> proposals = new LinkedHashSet<>();

        for (String property : type.getProperties().keySet()) {
            ProposalBuilder proposal = createPropertyProposal(property, type.getProperties().get(property));
            if (proposal != null) {
        ...
        for (String property : type.getPatternProperties().keySet()) {
            ...
            ProposalBuilder proposal = createPropertyProposal(property, typeDef);
            if (proposal != null) {
                proposals.add(proposal);
            }
        }
        ...
        if (type.getAdditionalProperties() != null) {
            ...
            if (elementName != null) {
                ...
                proposals.add(new ProposalBuilder(elementName).replacementString(elementName + ":").selection(elementName));
            }
        }
        if (proposals.isEmpty()) {
            proposals.add(new ProposalBuilder("_key_").replacementString("_key_" + ":"));
        }
        return proposals;
    }

    protected Collection<ProposalBuilder> createArrayProposals(ArrayTypeDefinition type, AbstractNode node) {
        Collection<ProposalBuilder> proposals = new LinkedHashSet<>();
        ...
        if (type.itemsType.getType() == JsonType.ENUM) {
            ...
            for (String literal : enumLiterals(type.itemsType)) {
                proposals.add(new ProposalBuilder(literal).replacementString("- " + literal).description(type.getDescription()).type(labelType));
            }
        } else {
            proposals.add(new ProposalBuilder("-").replacementString("-").description(type.getDescription()).type("array item"));
        }

        return proposals;
    }

    protected Collection<ProposalBuilder> createComplextTypeProposals(ComplexTypeDefinition type, AbstractNode node,
            String prefix) {
        final Collection<ProposalBuilder> proposals = new LinkedHashSet<>();
    ...
    protected Collection<ProposalBuilder> createEnumProposals(TypeDefinition type, AbstractNode node) {
        final Collection<ProposalBuilder> proposals = new LinkedHashSet<>();
        ...
        for (String literal : enumLiterals(type)) {
            ...
            proposals.add(new ProposalBuilder(literal).replacementString(replStr).description(type.getDescription()).type(labelType));
        }
